...
int main(int argc, char **argv) {
    ...
    b[0] = 0;
    atom_t c[2];
    c[1] = 2036752931478355632;
    c[0] = 0;
    unsigned char res;
    uint64_t  we= 1000000000000;
    uint64_t  s=we;
    //res =DCAS(a,b[0],b[1],c[0],c[1]);
    //res=abstraction_dcas(a, c, b);
    if(res == 1)
       cout<<res<<endl;
    cout << a[1] << endl;
    atom_t  ec =we;
    cout<<we<<endl;
    cout<<ec<<endl;
    //LocalAtomicEntryTest();
    stopMeasure.store(0, std::memory_order_relaxed);
    //LocalEntryTest();
    //uint64_t mhash =MurmurHash64A((uint8_t *) loads[i]->getKey(), std::strlen(loads[i]->getKey()), hashseedA);
    //cout<<buf[10]<<endl;
    //cout<< sizeof(a)<<endl;
